...
## Environment
```
conda env: physdreamer  (python 3.9, torch 2.0.0+cu118, warp 0.10.1)
...
## Entrypoints (each writes a self-contained, reproducible run dir)
```bash
# 1. forward: known E -> video
$ENV/bin/python -m reuse_mpm.forward_gen \
    --dataset_dir $PHYSDREAMER_ROOT/data/physics_dreamer/telephone \
    --E 1e5 --v0 0 -1 0 --num_frames 8 --substep 32 --out outputs/fwd_telephone_E1e5

# 2. identifiability / loss landscape (is E recoverable, over what range?)
$ENV/bin/python -m reuse_mpm.probe_identifiability \
    --dataset_dir .../telephone --v0 0 -1 0 --E_star 1e5 --E_min 1e3 --E_max 1e7 \
    --n 15 --out outputs/probe_v0_1

# 3. inverse: recover global E from a forward_gen run
$ENV/bin/python -m reuse_mpm.train_global_E \
    --gt_run outputs/fwd_telephone_E1e5 --init_E 3e5 --coarse_init \
    --iters 24 --lr 0.04 --window 3 --grad_window 1 --out outputs/inv_headline

# 4. dataset: sample E~p*(E)=logU[E_min,E_max] -> (E, video) dataset (sweep)
#    saves per-frame mpm_xyz.npy + per-sample stability flag (substep 96!)
$ENV/bin/python -m reuse_mpm.dataset_gen \
    --dataset_dir .../telephone --E_min 1e4 --E_max 1e6 --n 256 --substep 96 --seed 0 \
    --out outputs/dataset_telephone_256

# 5. honest recovery sweep (faithful PhysDreamer: substep 96, full BPTT, no cheat)
$ENV/bin/python -m reuse_mpm.recovery_sweep \
...
# 6. multi-scene forward+backward (PhysDreamer "pd:" + PhysGaussian "pg:")
$ENV/bin/python -m reuse_mpm.multiscene_fwdbwd \
...
`config.json`, `source_ply` (symlink), `frames/`, `video.mp4`/`.gif`, plus task
extras (`landscape.png`, `recovery.png`+`metrics.json`+`trace.json`,
dataset `manifest.json`+`p_star.png`+`sample_XXXX/`). Intermediate debug
