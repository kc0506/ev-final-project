...
@app.function(image=gic_image, gpu=GPU_RUN, timeout=5400,
              volumes={"/data": data_vol, "/out": out_vol, PD_DATASET_ROOT: pd_data_vol})
def fit_image_v0field(variant: str = "", scale: float = 1.0, lr: float = 0.025,
                      iters: int = 200, res: str = "4x4x16", frames: int = 14,
                      camera: str = "diag45", pan: float = 0.05, loge: float = 5.0,
                      mpm_iter: int = 64, gtvel: str = "0,-0.5,0", label: str = "") -> str:
    """v0 FIELD recovery from full-RES with-bg image loss (telephone).

    --variant "" (default) = UNIFORM GT field (--gtvel, the easy baseline that traj
    recovers to 1.7%) -> isolate whether image+field breaks on the field machinery
    itself before adding excitation structure. --variant true_bend|mid_kick|ramp_x =
    a non-uniform GT at the v/c-calibrated amplitude (--scale, use --loge 4.0). Recipe
    = a16 grid + TV 1e-3 + starve-freeze + 200 iter run-to-completion (TV keeps
    improving the flat subspace, final beats best -> patience disabled). lr is
    amplitude-scaled (uniform 0.5 -> 0.025, mid x10 -> 0.125)."""
    import subprocess
    import threading
    ...
    ply = f"{PD_DATASET_ROOT}/telephone/point_cloud.ply"
    assert os.path.exists(ply), f"point_cloud.ply missing (dataset_dir): {ply}"
    tag = label or variant or "uniform"
    out_dir = f"/out/img_v0field_{tag}"
    cmd = [
        GIC_PY, "fit_image_v0.py",
        "--scene.cache", cache,
        "--scene.config", "config/ours/telephone.json",
        "--scene.mpm-iter-cnt", str(mpm_iter),
        "--gt.logE", str(loge),
        "--vfield.res", res, "--vfield.lr", str(lr),
        "--render.camera", camera, "--render.cam-pan", str(pan),
        "--frames.n-frames", str(frames),
        "--train.iter-cnt", str(iters), "--train.patience", "999", "--train.min-iters", str(iters),
        "--out", out_dir,
    ]
    if gtvel and gtvel != "0,-0.5,0":  # default GTCfg.vel; pass only when overriding (avoids CLI negative-flag parsing)
        cmd += ["--gt.vel", *gtvel.split(",")]
    if variant:
        cmd += ["--gt-v0-variant", variant, "--gt-v0-scale", str(scale)]
    print("RUN:", " ".join(cmd), flush=True)
    stop = threading.Event()
    peak = [0]
    t = threading.Thread(target=lambda: peak.__setitem__(0, _gpu_mem_peak_mib(stop)[0]))
    ...
    result_path = os.path.join(out_dir, "result.json")
    ok = os.path.exists(result_path)
    msg = (f"gpu={GPU_RUN} tag={tag} cam={camera} exit={r.returncode} "
           f"whole_card_peak={peak[0]} MiB "
           f"result.json={'OK ' + result_path if ok else 'MISSING'}")
    print(msg)
    return msg
...
@app.function(image=gic_image, gpu=GPU_RUN, timeout=5400,
              volumes={"/data": data_vol, "/out": out_vol, PD_DATASET_ROOT: pd_data_vol})
def fit_image_cli(script: str = "fit_image_v0.py", args: str = "", out_name: str = "img_cli") -> str:
    """Generic fit_image_* runner: `python <script> --scene.cache <tele> --scene.config
    <tele.json> --out /out/<out_name> <args>`. The free-form --args string (shlex-split)
    carries the render/gt/vfield/train flags so any fit_image_* mode (v0 scalar/field,
    joint, multiview, az45 lookat, with/without bg) runs without a bespoke entrypoint.
    Code is runtime-mounted so local edits (lookat camera, no-bg fullres, ...) apply."""
    import shlex
    import subprocess
    import threading

    cache = "/data/telephone_ds0.1_g32_k8.pt"
    assert os.path.exists(cache), f"cache missing: {cache}"
    ply = f"{PD_DATASET_ROOT}/telephone/point_cloud.ply"
    assert os.path.exists(ply), f"point_cloud.ply missing: {ply}"
    out_dir = f"/out/{out_name}"
    cmd = ([GIC_PY, script, "--scene.cache", cache,
            "--scene.config", "config/ours/telephone.json", "--out", out_dir]
           + shlex.split(args))
    print("RUN:", " ".join(cmd), flush=True)
    stop = threading.Event()
    peak = [0]
    ...
    result_path = os.path.join(out_dir, "result.json")
    ok = os.path.exists(result_path)
    msg = (f"gpu={GPU_RUN} {out_name} exit={r.returncode} whole_card_peak={peak[0]} MiB "
           f"result.json={'OK ' + result_path if ok else 'MISSING'}")
    print(msg)
    return msg
